...
/* ── S10 · proven, not promised ─────────────────────────────────────────── */
{
  ...
  const cards = [
    ["565 gate scripts", "the suite runs on every push — plus determinism, cache-transparency and golden contracts; the gate count itself is gated against the runner's own loop"],
...
/* ── S10b · why the claims are worth anything ───────────────────────────── */
{
  ...
  stat(s, "565", "gate scripts named by test/regression.sh — and the COUNT itself is gated against the runner's own loop, so it cannot go stale quietly",
...
/* ── S12c · re-derive this deck ─────────────────────────────────────────── */
{
  ...
  const claims = [
    ...
    ["565 gate scripts",                  "bash test/manifestcheck.sh"],
