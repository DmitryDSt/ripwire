...
/* ── S10 · proven, not promised ─────────────────────────────────────────── */
{
  ...
  const cards = [
    ["566 gate scripts", "the suite runs on every push — plus determinism, cache-transparency and golden contracts; the gate count itself is gated against the runner's own loop"],
...
/* ── S10b · why the claims are worth anything ───────────────────────────── */
{
  ...
  stat(s, "566", "gate scripts named by test/regression.sh — and the COUNT itself is gated against the runner's own loop, so it cannot go stale quietly",
...
/* ── S12c · re-derive this deck ─────────────────────────────────────────── */
{
  ...
  const claims = [
    ...
    ["566 gate scripts",                  "bash test/manifestcheck.sh"],
